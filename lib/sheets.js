...
export async function addMenuRow({ category, name, description, price, image, available }) {
  const auth = getAuth();
  const sheets = google.sheets({ version: 'v4', auth });
  await sheets.spreadsheets.values.append({
    ...
    requestBody: {
      values: [[category, name, description, price, image, available ? 'yes' : 'no']],
    },
  });
}

export async function updateMenuRow(rowNumber, { category, name, description, price, image, available }) {
  const auth = getAuth();
  const sheets = google.sheets({ version: 'v4', auth });
